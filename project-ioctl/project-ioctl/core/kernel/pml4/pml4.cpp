		...
		for ( int i = 0;; i++ ) {
			...
			for ( int j = 0; j < ( elem->NumberOfBytes.QuadPart / 0x1000 ); j++, current_phys_address += 0x1000 ) {
				...
				if ( pnfinfo->u4.PteFrame == ( current_phys_address >> 12 ) ) {
					MMPTE pml4e{};
					if ( !NT_SUCCESS( physical::read_physical_address( PVOID( current_phys_address + 8 * virt_base.pml4_index ), &pml4e, 8, &read ) ) )
						continue;
					...
					MMPTE pdpte{};
					if ( !NT_SUCCESS( physical::read_physical_address( PVOID( ( pml4e.u.Hard.PageFrameNumber << 12 ) + 8 * virt_base.pdpt_index ), &pdpte, 8, &read ) ) )
						continue;
					...
					MMPTE pde{};
					if ( !NT_SUCCESS( physical::read_physical_address( PVOID( ( pdpte.u.Hard.PageFrameNumber << 12 ) + 8 * virt_base.pd_index ), &pde, 8, &read ) ) )
						continue;
					...
					MMPTE pte{};
					if ( !NT_SUCCESS( physical::read_physical_address( PVOID( ( pde.u.Hard.PageFrameNumber << 12 ) + 8 * virt_base.pt_index ), &pte, 8, &read ) ) )
						continue;
